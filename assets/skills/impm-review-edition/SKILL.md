...
## 何时使用
用户要求执行 impm 软件工程全流程开发，并希望在项目初始化阶段对初始化文档、需求分析整理阶段对每份文档（URS、PRD、SAD、DBD、API、LLD、任务清单）生成后由用户逐份审核确认时使用。本技能与 impm 技能流程完全一致，唯一区别是将阶段一由 impm-init 替换为 impm-init-review（初始化文档逐份审核），将阶段二由 impm-docs 替换为 impm-docs-review。

## 执行角色
本技能由 项目经理（主控 Agent） 负责执行（编排），执行时使用 Skill 工具加载本技能。内部子步骤必须按下方「通用调度要求」派发对应 subagent 执行，PM 只调度、检查与决策；**阶段一与阶段二中的用户文档审核确认（含摘要提取与展示）由 PM 直接执行，不派发 subagent**。

## 通用调度要求（本技能所有子步骤必须遵守）
1. 启动方式：每个子步骤通过 task 工具启动对应 subagent（subagent_type 必须与下方对照表完全一致）执行对应技能；禁止 PM 自己代替 subagent 执行具体事务（唯一例外：阶段一、阶段二的用户文档审核确认环节由 PM 直接执行）。
...
|----|----|----|
| 阶段一 | impm-init-review | PM（编排，内部再派发，含逐文档用户审核） |
...
5. **阶段一 impm-init-review 与阶段二 impm-docs-review 中的文档审核都必须在每份文档生成后先提取摘要展示在对话框中，再弹出提示框等待用户明确确认（审核通过）才能进入下一步**；审核未通过时按用户反馈重新生成再审，不得跳过。
6. 全程使用简体中文。

## 执行步骤

### 阶段一：项目初始化（impm-init-review，含逐文档用户审核）
...
2. 使用 Skill 工具加载并执行 impm-docs-review 技能，依次执行：impm-version-create → impm-urs-create（审核）→ impm-prd-create（审核）→ impm-sad-update（审核）→ impm-dbd-create（审核）→ impm-api-create（审核）→ impm-lld-create（审核）→ impm-task-create（审核）→ impm-rtm-create → impm-analysis-commit。其中标记"（审核）"的步骤完成后均由 PM 先提取文档摘要展示在对话框中，再弹出提示框请用户审核文档，审核通过后才进入下一步。
...
3. 向用户汇报本次版本开发的完整产出，并说明文档审核结果（阶段一初始化文档与阶段二各文档的审核确认情况）。
...
- 阶段一初始化文档与阶段二各文档的用户审核确认记录（对话内完成，摘要不落盘）
