...
1. **识别服务与中间件**：
   - 分析项目结构，识别被测服务及其依赖的中间件（如数据库、消息队列、缓存、外部 API 等）。
   - 查找项目根目录下的配置文件（如 `.env`、`config.*`、`docker-compose.yml`、`Makefile`、`scripts/` 目录等），确定服务的启动方式和依赖关系。
   - 如有 `docker-compose.yml`，优先使用 Docker Compose 管理服务与中间件。

2. **检查服务状态**：
   - 检查被测服务是否已启动：尝试访问服务健康检查端点（如 `http://localhost:端口/health`）或检查进程/端口。
   - 检查依赖中间件是否已启动：尝试连接数据库、消息队列等，或检查对应进程/端口。
   - 记录检查结果：哪些服务/中间件已启动，哪些未启动。

3. **启动未运行的服务与中间件**：
   - 若存在 `docker-compose.yml`，使用 `docker-compose up -d` 启动所有服务与中间件。
   - 若存在启动脚本（如 `scripts/start.sh`、`Makefile` 中的 `start` 目标），执行相应命令。
   - 若服务为独立进程，根据项目文档或配置文件中的说明启动。
   - 启动后，等待服务就绪（如等待端口监听、健康检查返回 200 等），设置合理超时（如 30 秒）。

4. **验证启动状态**：
   - 重新检查所有服务与中间件是否已成功启动。
   - 若仍有服务未启动，记录错误并终止接口测试，向调度方报告启动失败原因。
...
3. 用步骤 4.0 确定的 python 运行命令依次调用 docs/api-test/run_api_test.py 运行版本目录 docs/api-test/{项目英文缩写}-v{当前版本号}/ 下的每个集合文件（可加 `--base-url http://localhost:端口` 指定被测服务地址），读取其生成的 docs/api-test/{项目英文缩写}-v{当前版本号}/api-test-report.json 汇总结果，记录每个集合的执行结果（通过/失败、断言明细、错误信息）。
...
### 步骤 4.5：处理过时测试用例（仅接口测试）
1. 分析步骤 4 中失败的接口测试用例，识别失败原因为"用例已过时"（如接口已下线、请求/响应格式已变更、业务逻辑已调整导致用例不再适用）的测试用例。
2. 若存在过时测试用例，使用 question 工具向用户列出这些测试用例：
   - 测试用例编号（TC-{版本号}-{任务编号}-{序号}）
   - 测试用例名称
...
### 步骤 4.6：写入接口测试回归结果
1. 将步骤 4 的测试结果整理为 Markdown 文档，内容包括：集合名称、执行命令、用例数、通过数、失败数、失败明细、过时用例删除记录（如有）、结论。
...
6. 调用 impm_doc_writer（docType=rtm，target=version，expectedBase=步骤 5.1 读取到的最新全文）将包含测试用例关联、覆盖校验与问题清单的完整 rtm.md 覆盖写回（若本版本原本无 rtm.md，可新建，或按 main 写入主文档）。
...
2. 汇总阶段一所需测试指标：
   ...
   - 接口测试：用例总数、通过数、失败数、通过率（取自步骤 4/4.6 的回归结果）；
...
4. 调用 impm_doc_writer（docType=regression，target=version，expectedBase=步骤 6.3 读取到的最新全文）写入 docs/{项目英文缩写}-v{当前版本号}/regression.md；审核质量度量章节（第 3~5 章）暂留待阶段二回填。
