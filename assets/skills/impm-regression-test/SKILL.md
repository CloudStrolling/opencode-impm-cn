...
description: 执行版本回归测试，全量运行单元测试和接口测试并记录结果，识别并删除过时测试用例，对需求追踪矩阵（RTM）进行测试用例回填与覆盖完整度校验，并生成当前版本质量度量报告（阶段一：测试度量，写入 regression.md，审核类质量指标由 impm-regression-metrics 阶段二回填）
---

# impm-regression-test 技能

## 触发词
回归测试、单元测试、接口测试、需求追踪矩阵、RTM、覆盖校验、测试覆盖率、质量度量、缺陷指标、regression
...
### 步骤 1.5：处理过时测试用例（仅单元测试）
1. 分析步骤 1 中失败的测试用例，识别失败原因为"用例已过时"（如测试的功能点已下线、接口已废弃、业务逻辑已变更导致用例不再适用）的测试用例。
2. 若存在过时测试用例，使用 question 工具向用户列出这些测试用例：
   - 测试用例编号（TC-{版本号}-{任务编号}-{序号}）
   - 测试用例名称
   - 所在测试文件路径
   - 过时原因
3. 询问用户是否确认删除这些过时的测试用例。
4. 若用户确认删除：
   a. 从版本测试用例文档 docs/{项目英文缩写}-v{当前版本号}/{项目英文缩写}-testcase-v{当前版本号}.md 中删除对应的测试用例条目；
   b. 删除测试用例对应的测试代码文件或测试函数；
   c. 记录删除的测试用例清单。

### 步骤 2：写入单元测试回归结果
1. 将步骤 1 的测试结果整理为 Markdown 文档，内容包括：测试时间、运行环境、执行命令、统计结果、失败用例清单及失败原因、过时用例删除记录（如有）、结论。
...
1. 确认 docs/api-test/ 目录下存在接口测试执行程序 run_api_test.py；若不存在，则从 assets/skills/template/API-TEST-RUNNER.py 模板复制到该路径。
2. 列出版本目录 docs/api-test/{项目英文缩写}-v{当前版本号}/ 下的全部 Postman Collection v2.1 接口测试用例文件（如 {项目英文缩写}-api-test-v{当前版本号}.postman_collection.json）。
3. 用步骤 3.0 确定的 python 运行命令依次调用 docs/api-test/run_api_test.py 运行版本目录 docs/api-test/{项目英文缩写}-v{当前版本号}/ 下的每个集合文件（可加 `--base-url http://localhost:端口` 指定被测服务地址），读取其生成的 docs/api-test/{项目英文缩写}-v{当前版本号}/api-test-report.json 汇总结果，记录每个集合的执行结果（通过/失败、断言明细、错误信息）。
4. 遍历所有版本：如需对历史版本做整体回归，可遍历 docs/api-test/ 下的全部 {项目英文缩写}-v{版本号}/ 目录，对每个版本的集合依次执行（每个版本的报告分别写入对应版本目录 docs/api-test/{项目英文缩写}-v{版本号}/）。本步骤默认只对当前版本执行回归。
...
2. 若存在过时测试用例，使用 question 工具向用户列出这些测试用例：
   - 测试用例编号（TC-{版本号}-{任务编号}-{序号}）
   - 测试用例名称
   - 所在 Postman Collection 文件路径
   - 过时原因
3. 询问用户是否确认删除这些过时的测试用例。
4. 若用户确认删除：
   a. 从版本测试用例文档 docs/{项目英文缩写}-v{当前版本号}/{项目英文缩写}-testcase-v{当前版本号}.md 中删除对应的测试用例条目；
   b. 从 Postman Collection JSON 文件中删除对应的 item；
   c. 记录删除的测试用例清单。

### 步骤 3.6：写入接口测试回归结果
1. 将步骤 3 的测试结果整理为 Markdown 文档，内容包括：集合名称、执行命令、用例数、通过数、失败数、失败明细、过时用例删除记录（如有）、结论。
2. 调用 impm_doc_writer（docType=regression-api）写入 docs/{项目英文缩写}-v{当前版本号}/regression-api-test.md。
3. 核对文件存在且内容正确。

### 步骤 4：需求追踪矩阵（RTM）测试用例回填与覆盖校验
...
2. 收集本版本测试用例：读取当前版本测试用例 docs/{项目英文缩写}-v{当前版本号}/{项目英文缩写}-testcase-v{当前版本号}.md，提取所有测试用例编号（TC-xxx）及其"关联需求ID"（FR-xxx / US-xxx；需求/用户故事编号项目全局唯一，形如 FR-v0.0.1-001、US-v0.0.1-001）。
...
4. 覆盖完整度校验：逐条核对"一、需求/用户故事清单"中的每个原始需求（FR-xxx / NFR-xxx）与用户故事（US-xxx，编号均为全局唯一格式 FR-v{版本号}-xxx / US-v{版本号}-xxx）是否同时满足以下全部条件：
...
6. 调用 impm_doc_writer（docType=rtm，target=version，expectedBase=步骤 4.1 读取到的最新全文）将包含测试用例关联、覆盖校验与问题清单的完整 rtm.md 覆盖写回（若本版本原本无 rtm.md，可新建，或按 main 写入主文档）。
7. 核对 rtm.md 已写入且校验结果正确。

### 步骤 5：生成版本质量度量报告（阶段一：测试度量）
...
2. 汇总阶段一所需测试指标：
   - 单元测试：用例总数、通过数、失败数、通过率（取自步骤 1/2 的回归结果）；
   - 接口测试：用例总数、通过数、失败数、通过率（取自步骤 3/3.6 的回归结果）；
   - 测试覆盖率：需求/用户故事用例覆盖（依据步骤 4 RTM 覆盖完整度校验结果计算：已覆盖的需求/用户故事数 / 需求/用户故事总数）；如工具可产出代码/分支覆盖率一并填入，无则标注"不适用"。
3. 读取当前版本 regression.md（docType=regression，target=version）；若不存在则按模板新建，若已存在则保留历史内容与后续阶段二已回填部分，仅更新阶段一测试度量部分。
4. 调用 impm_doc_writer（docType=regression，target=version，expectedBase=步骤 5.3 读取到的最新全文）写入 docs/{项目英文缩写}-v{当前版本号}/regression.md；审核质量度量章节（第 3~5 章）暂留待阶段二回填。
