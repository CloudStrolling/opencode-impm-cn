...
## 何时使用
在需求追踪矩阵生成完成（impm-rtm-create 之后）时使用，作为需求分析整理阶段（阶段2）的最后一步：将本阶段生成的所有文件和目录（含 rtm.md）提交到 git，记录进度，并向用户报告需求分析阶段全部完成。
...
### 步骤 2：提交到 git
调用 impm_git（action=commit，message={项目英文缩写}-v{当前版本号}-需求分析整理）将需求分析整理阶段生成的所有文件和目录提交到 git。提交内容包括：版本目录 docs/{项目英文缩写}-v{当前版本号}/ 下生成的各文档（含 rtm.md）、任务清单、进度文件，以及本阶段更新的主文档（如 docs/{项目英文缩写}-sad.md 的修改）。确认提交成功。
