...
以下"文档审核确认"环节在 urs/prd/sad/dbd/api/lld/task 这 7 个文档生成步骤完成后触发，包含"文档摘要提取与展示"和"弹出审核提示框"两步。PM 通过 question 工具弹出提示框，请用户审核已生成的文档。

### 第一步：文档摘要提取与展示（弹窗前置，PM 直接执行）
在每次触发审核提示框之前，PM 必须先提取待审核文档的简明摘要，并以普通文本消息直接显示在当前对话中，供用户快速预览。要求：
1. 读取文档：用 read 工具（或 impm_doc_reader）读取刚生成的文档全文（优先读取版本目录内完整版本文档 docs/{项目英文缩写}-v{当前版本号}/{项目英文缩写}-{文档简称}-v{当前版本号}.md，仅主文档的读取主文档）。
2. 提取摘要：归纳出简明摘要，一般包括：文档标题与完整路径、文档章节结构、核心内容要点（如需求目标与关键需求/用户故事、架构决策、数据表/字段清单、接口清单、任务清单规模与分类等）。
...
2. 若用户选择"需要修改"：
   ...
   - 子步骤完成后再次执行"第一步：文档摘要提取与展示"（重新提取新文档摘要展示在对话框中），并弹出提示框请用户审核，直到用户选择"审核通过"为止（连续多次未通过可先与用户确认修改意见的可行性，避免无谓反复）；
   - 期间不得推进到下一步骤。
...
3. **文档审核确认（URS）**：按上文"用户文档审核确认环节"（先提取文档摘要展示在对话框中，再弹出提示框）请用户审核 URS 文档；审核通过后方可进入下一步。
...
3. **文档审核确认（PRD）**：按上文"用户文档审核确认环节"（先提取文档摘要展示在对话框中，再弹出提示框）请用户审核 PRD 文档；审核通过后方可进入下一步。
...
3. **文档审核确认（SAD）**：若本步骤实际更新/产出了 SAD 文档（状态为"已完成"且 docs/{项目英文缩写}-sad.md 有实质更新），按上文"用户文档审核确认环节"（先提取文档摘要展示在对话框中，再弹出提示框）请用户审核；若状态为"无需修改"且未产出新内容，则不弹窗，直接进入下一步。
...
3. **文档审核确认（DBD）**：若实际产出 DBD 文档（状态为"已完成"），按上文"用户文档审核确认环节"（先提取文档摘要展示在对话框中，再弹出提示框）请用户审核；若"无需数据库"则不弹窗，进入下一步。
...
3. **文档审核确认（API）**：若实际产出 API 文档（状态为"已完成"），按上文"用户文档审核确认环节"（先提取文档摘要展示在对话框中，再弹出提示框）请用户审核；若"无需接口"则不弹窗，进入下一步。
...
3. **文档审核确认（LLD）**：按上文"用户文档审核确认环节"（先提取文档摘要展示在对话框中，再弹出提示框）请用户审核 LLD 文档；审核通过后方可进入下一步。
...
3. **文档审核确认（任务清单）**：按上文"用户文档审核确认环节"（先提取任务清单摘要展示在对话框中，再弹出提示框）请用户审核任务清单；审核通过后方可进入下一步。
