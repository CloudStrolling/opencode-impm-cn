---
name: impm-init
description: 编排初始化阶段全部13个步骤（impm-init-isinit、impm-init-git、impm-init-project、impm-init-version、impm-init-urs、impm-init-prd、impm-init-sad、impm-init-dbd、impm-init-api、impm-init-lld、impm-init-task、impm-init-testcase、impm-init-commit），严格按顺序依次执行并汇报结果。空项目在创建 project.md 后跳过后续初始化步骤，直接进入 impm-docs。当用户输入 /impm-init 或要求初始化 impm 工程时使用。
---
...
| 子步骤 | 技能名 | subagent_type | 备注 |
|----|----|----|----|
| a | impm-init-isinit | PM 直接执行 | |
| b | impm-init-git | scm | |
| c | impm-init-project | sa | |
| d | impm-init-version | sa | 空项目跳过 |
| e | impm-init-urs | ba | 空项目跳过 |
| f | impm-init-prd | ba | 空项目跳过 |
| g | impm-init-sad | sa | 空项目跳过 |
| h | impm-init-dbd | dba | 空项目跳过 |
| i | impm-init-api | sa | 空项目跳过 |
| j | impm-init-lld | tl | 空项目跳过 |
| k | impm-init-task | tl | 空项目跳过 |
| l | impm-init-testcase | te | 空项目跳过 |
| m | impm-init-commit | scm | 空项目跳过 |
...
## 执行要求
1. 严格按照执行步骤中的内容和顺序依次执行：不跳过、不乱序、不并行、不合并任何步骤（**空项目例外：步骤 c 完成后跳过步骤 d-m，直接进入 impm-docs**）。
...
- 若为空项目：按空项目初始化，各文档按模板写入空结构。**空项目在完成步骤 b（git）和步骤 c（project.md）后将跳过步骤 d-m，直接进入 impm-docs 需求分析整理阶段。**
- 若为存量项目：按存量项目反推补全各文档。
...
- **若为存量项目**：继续顺序执行后续步骤 d-m（版本初始化 → URS → PRD → SAD → DBD → API → LLD → 任务清单 → 测试用例 → 最终提交）。
- **若为空项目**：跳过步骤 d-m，**直接进入 impm-docs 需求分析整理阶段**（使用 Skill 工具加载并执行 impm-docs 技能）。空项目的版本目录、版本进度表、URS、PRD、SAD、DBD、API、LLD、任务清单、测试用例等文档将由 impm-docs 在后续版本迭代中按需创建。跳转前 PM 向用户说明：空项目初始化已完成（git 基线 + project.md），将直接进入需求分析整理阶段。
...
### 步骤 e：执行 impm-init-urs（用户需求说明书，仅存量项目）
...
### 步骤 f：执行 impm-init-prd（产品需求文档，仅存量项目）
...
### 步骤 g：执行 impm-init-sad（系统架构设计，仅存量项目）
...
### 步骤 h：执行 impm-init-dbd（数据库设计，仅存量项目）
...
### 步骤 i：执行 impm-init-api（接口设计，仅存量项目）
...
### 步骤 j：执行 impm-init-lld（详细设计，仅存量项目）
...
### 步骤 k：执行 impm-init-task（任务清单，仅存量项目）
...
调用 impm_progress(projectRoot, {项目英文缩写}, {当前版本号}, add, impm-init, 已完成) 记录本编排技能完成；随后调用 impm_progress(projectRoot, {项目英文缩写}, {当前版本号}, finalize, null, null) 在退出前结算进度表最后一行（impm-init，已完成）的总耗时与 token（含该步骤主会话与 subagent 子会话消耗）；向用户汇报初始化阶段全部完成，汇报内容包括：初始化方式（存量项目）、产出文件清单、各步骤执行角色（PM/SCM/SA/BA/DBA/TL/TE）、版本进度表位置与下一步建议（进入 impm-docs 需求分析整理阶段）。

### 空项目完成汇报
若为空项目，步骤 c 完成后 PM 直接向用户汇报空项目初始化完成，汇报内容包括：已完成的步骤（impm-init-isinit、impm-init-git、impm-init-project）、产出文件（docs/project.md、.gitignore、git 初始提交）、下一步建议（直接进入 impm-docs 需求分析整理阶段）。

## 交付物
- **存量项目**：版本目录 docs/{项目英文缩写}-v0.0.1/ 及其全部初始化文档、主文档（project.md、sad.md、urs.md、prd.md、api.md、dbd.md、dbd.sql、lld.md、testcase.md）、任务清单、版本进度表 version_progress.md、.gitignore 与 git 初始提交记录、接口测试用例与执行器
- **空项目**：docs/project.md、.gitignore 与 git 初始提交记录

## 完成后提示
- **存量项目**：初始化阶段已全部完成，下一步进入 impm-docs 需求分析整理阶段（输入 /impm-docs）。如需查看版本进度，请查看版本目录下的 version_progress.md。
- **空项目**：空项目初始化已完成（git + project.md），将直接进入 impm-docs 需求分析整理阶段。
