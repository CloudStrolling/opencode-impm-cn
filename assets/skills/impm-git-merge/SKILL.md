...
## 执行步骤
### 步骤 0：前置条件检查（回归测试与代码审核）
1. 读取当前版本的回归测试报告 `docs/{项目英文缩写}-v{当前版本号}/regression.md`，检查测试结果是否全部通过。
2. 读取当前版本的代码审核报告 `docs/{项目英文缩写}-v{当前版本号}/review.md`，检查是否存在未修复的严重/高危问题。
3. **任一条件不满足时，立即停止执行**，向用户报告：
   - 回归测试未通过的具体用例或失败原因
   - 代码审核中未修复的严重/高危问题清单
   - 提示用户先处理上述问题，确认无误后手动执行合并
4. 两项均通过后，方可继续执行后续步骤。
