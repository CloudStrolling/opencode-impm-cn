...
description: 读取 TESTCASE-TEMPLATE.MD 模板，根据项目代码、文档及 PRD、LLD 确定测试用例，编写单元测试函数并生成 Postman Collection v2.1 接口测试用例（版本目录 docs/api-test/{项目英文缩写}-v{当前版本号}/，配合 docs/api-test/run_api_test.py 执行）。当初始化阶段需要编写测试用例时使用。
---
...
针对测试用例中的 API 接口测试，生成 Postman Collection v2.1 格式的 JSON 用例文件，放入版本目录 docs/api-test/{项目英文缩写}-v0.0.1/{项目英文缩写}-api-test-v0.0.1.postman_collection.json，并与接口测试执行程序配套使用：
1. 确认 docs/api-test/ 目录下已存在接口测试执行程序 run_api_test.py；若不存在，则从 assets/skills/template/API-TEST-RUNNER.py 模板复制到该路径（即创建 run_api_test.py）。
...
3. 接口测试通过 `python docs/api-test/run_api_test.py docs/api-test/{项目英文缩写}-v0.0.1/{项目英文缩写}-api-test-v0.0.1.postman_collection.json --report-dir docs/api-test/{项目英文缩写}-v0.0.1` 执行，由程序读取集合、逐个发送请求、对比 expected 预期结果并生成测试报告（控制台 + docs/api-test/{项目英文缩写}-v0.0.1/api-test-report.md、api-test-report.json）。**运行前先检测 Python 环境**（运行 .py 接口测试程序依赖 python 环境）：
...
- docs/api-test/{项目英文缩写}-v0.0.1/{项目英文缩写}-api-test-v0.0.1.postman_collection.json（Postman Collection v2.1 接口测试用例）
- docs/api-test/run_api_test.py（接口测试执行程序，由模板复制）
