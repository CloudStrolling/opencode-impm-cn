...
description: 单个编码任务的步骤蓝本（context→cs→ws→dbd→api→testcase→code→writetest→runtest），由 PM 在 impm-coding 的阶段波次并发调度中直接执行，并遵守版本目录写入冲突规避规则。
---
...
## 何时使用
impm-coding（PM）确定某任务可执行（无前置任务或前置任务全部"已完成"）并纳入并发批次后，需要按标准步骤序列完成该任务的编码开发时使用。本技能描述**单个任务**的步骤蓝本；PM 在 impm-coding 中按"阶段波次"模型执行：同一阶段为批次内多个任务并发派发对应子步骤 subagent，本技能定义每个任务的步骤顺序与各阶段的 subagent 类型、写入冲突规避规则。

## 执行角色
本技能由 项目经理（主控 Agent，PM） 负责执行（编排），执行时使用 Skill 工具加载本技能。内部子步骤按下方「通用调度要求」派发对应 subagent 执行，PM 只调度、检查与决策。**不引入 TM 角色**：impm-coding 直接按本技能的阶段顺序为批次内各任务并发派发子步骤 subagent。
...
3. task 提示词模板（每个子步骤照此填写）：
   "以 {subagent 中文名}（subagent_type={x}）身份执行 impm 的 {技能名} 技能；先用 Skill 工具加载技能 {技能名}；必须携带的上下文：项目根目录={绝对路径}、项目英文缩写={缩写}、当前版本号={版本号}、用户输入={原文}、任务编号={taskId}；注意：本子步骤可能与其他任务的同阶段子步骤并行执行，写入版本目录共享文档时必须遵守「版本目录写入冲突规避」规则（先读最新、合并、expectedBase 写回、冲突重试）；按技能执行步骤完成全部操作后，返回：产出文件路径清单与 version_progress.md 中 {技能名} 的进度状态。"
4. 完成核对：每个 subagent 返回后，核对产出文件存在、内容正确、version_progress.md 已记录（{任务编号} 前缀），全部正确才能进入下一阶段。
5. 顺序纪律：任务内部各阶段严格执行顺序，不跳过、不乱序、不合并；任一子步骤失败时先定位原因，必要时回退重做，不得绕过。**任务内部阶段之间禁止并行**（并行只发生在任务之间，由 impm-coding 统一调度）。

### 子步骤 subagent 对照表（impm-task-coding 单任务阶段序列）
...
多个任务并行编码时，任务会向版本目录写入内容，存在并发写冲突。**所有存在写入冲突的地方及规避规则如下**：
...
| scripts/API-TEST/{项目英文缩写}-api-test-v{当前版本号}.py | writetest 阶段 | 多任务并发覆盖写 | 同上：先读最新脚本，保留他人测试函数与入口，仅新增本任务测试函数并注册入口，expectedBase 写回，冲突重试 |
...
## 执行要求
1. 严格按照本技能阶段顺序执行：不跳过、不乱序、不合并任何阶段（任务内部串行，任务之间由 impm-coding 并发）。
2. 只执行本技能规定的操作，不做与任务无关的工作。
3. 所有文档路径必须用 {项目英文缩写}、{当前版本号}、{任务编号} 拼接，不得臆造文件名。
4. 使用 impm_* 工具获取信息，不得编造工具返回结果。
5. 全程使用简体中文。
6. 每个阶段完成后，核对产出文件是否存在、内容是否正确。
7. 本技能为编码流程的一部分，由 impm-coding（PM）按阶段波次直接执行，不能单独脱离版本号和任务编号执行。

## 执行步骤（单任务阶段序列）
### 步骤 1：接收版本号与任务编号
接收调度方（impm-coding/PM）传入的当前版本号与任务编号（{任务编号}，如 TASK-001），调用 impm_task_manager（action=query，taskId={任务编号}）校验任务存在、状态为"执行中"。
...
### 步骤 6：阶段 4 处理数据库设计
判断项目是否需要数据库（impm_doc_reader docType=dbd，target=main 是否存在）；需要时启动 DBA subagent 执行 impm-task-coding-dbd 技能，判断数据库设计是否需要变更并同步更新版本数据库设计文档与 SQL 脚本；写入必须遵守「版本目录写入冲突规避」规则。
