...
### 步骤 2：读取模板
调用 impm_template_reader(projectRoot, DBD-TEMPLATE.MD) 读取数据库设计文档模板，明确模板章节：ER 图、物理模型（按子系统/业务模块分组）、索引设计、数据字典、数据初始化等。

### 步骤 3：反推数据库设计
通过 impm_doc_reader 读取已有文档（重点是 PRD、SAD，以及 PRD 中的数据需求），结合当前项目代码与文档，按模板格式填写 DBD，同时反推项目初始化的 SQL 语句（建库、建表、初始数据等）：
- 存量项目：从现有代码、实体类、持久层代码反推表结构与索引设计，按子系统/业务模块分组组织物理模型，表名遵循 `{子系统缩写}_{模块缩写}_{实体名}` 命名规范。
- 空项目：写空 MD 和空 SQL（保留模板结构）。
- 数据初始化：识别项目中需要预置的数据（枚举字典、配置项、初始业务数据），在 DBD 文档第 11 章和 SQL 脚本中同步写入。
