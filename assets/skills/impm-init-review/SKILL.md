...
| 子步骤 | 技能名 | subagent_type | 是否需用户审核 | 备注 |
|----|----|----|----|----|
| a | impm-init-isinit | PM 直接执行 | 否 | |
| b | impm-init-git | scm | 否 | |
| c | impm-init-project | sa | 是 | |
| d | impm-init-version | sa | 否 | 空项目跳过 |
| e | impm-init-urs | ba | 是 | 空项目跳过 |
| f | impm-init-prd | ba | 是 | 空项目跳过 |
| g | impm-init-sad | sa | 是 | 空项目跳过 |
| h | impm-init-dbd | dba | 是 | 空项目跳过 |
| i | impm-init-api | sa | 是 | 空项目跳过 |
| j | impm-init-lld | tl | 是 | 空项目跳过 |
| k | impm-init-task | tl | 是 | 空项目跳过 |
| l | impm-init-testcase | te | 是 | 空项目跳过 |
| m | impm-init-commit | scm | 否 | 空项目跳过 |
...
## 执行要求
1. 严格按照执行步骤中的内容和顺序依次执行：不跳过、不乱序、不并行、不合并任何步骤（**空项目例外：步骤 c 完成后跳过步骤 d-m，直接进入 impm-docs-review**）。
...
2. 若用户选择"需要修改"：
   - **PM 立即在对话框中询问用户具体的修改意见**（如："请告诉我需要修改哪些内容？"），等待用户回复；
...
- 若为空项目：按空项目初始化，各文档按模板写入空结构。**空项目在完成步骤 b（git）和步骤 c（project.md，含审核）后将跳过步骤 d-m，直接进入 impm-docs-review 需求分析整理阶段。**
- 若为存量项目：按存量项目反推补全各文档。
...
### 步骤 c 完成后分支判断
根据步骤 a 传入的判定结果（项目类型）执行分支：
- **若为存量项目**：继续顺序执行后续步骤 d-m（版本初始化 → URS → PRD → SAD → DBD → API → LLD → 任务清单 → 测试用例 → 最终提交，其中标记"（审核）"的步骤完成后均需用户审核）。
- **若为空项目**：跳过步骤 d-m，**直接进入 impm-docs-review 需求分析整理阶段**（使用 Skill 工具加载并执行 impm-docs-review 技能）。空项目的版本目录、版本进度表、URS、PRD、SAD、DBD、API、LLD、任务清单、测试用例等文档将由 impm-docs-review 在后续版本迭代中按需创建。跳转前 PM 向用户说明：空项目初始化已完成（git 基线 + project.md），将直接进入需求分析整理阶段。

### 步骤 d：执行 impm-init-version（版本初始化，仅存量项目）
...
### 步骤 e：执行 impm-init-urs（用户需求说明书，仅存量项目，含审核）
...
### 步骤 f：执行 impm-init-prd（产品需求文档，仅存量项目，含审核）
...
### 步骤 g：执行 impm-init-sad（系统架构设计，仅存量项目，含审核）
...
### 步骤 h：执行 impm-init-dbd（数据库设计，仅存量项目，含审核）
...
### 步骤 i：执行 impm-init-api（接口设计，仅存量项目，含审核）
...
### 步骤 j：执行 impm-init-lld（详细设计，仅存量项目，含审核）
...
### 步骤 k：执行 impm-init-task（任务清单，仅存量项目，含审核）
...
### 步骤 l：执行 impm-init-testcase（测试用例与测试脚本，仅存量项目，含审核）
...
### 步骤 n：记录编排完成并汇报（仅存量项目）
调用 impm_progress(projectRoot, {项目英文缩写}, {当前版本号}, add, impm-init-review, 已完成) 记录本编排技能完成；随后调用 impm_progress(projectRoot, {项目英文缩写}, {当前版本号}, finalize, null, null) 在退出前结算进度表最后一行（impm-init-review，已完成）的总耗时与 token（含该步骤主会话与 subagent 子会话消耗）；向用户汇报初始化阶段全部完成，汇报内容包括：初始化方式（存量项目）、产出文件清单、各步骤执行角色（PM/SCM/SA/BA/DBA/TL/TE）、各文档审核结果、版本进度表位置与下一步建议（进入 impm-docs-review 需求分析整理阶段）。

### 空项目完成汇报
若为空项目，步骤 c（含 project.md 审核通过）完成后 PM 直接向用户汇报空项目初始化完成，汇报内容包括：已完成的步骤（impm-init-isinit、impm-init-git、impm-init-project（含审核））、产出文件（docs/project.md、.gitignore、git 初始提交）、下一步建议（直接进入 impm-docs-review 需求分析整理阶段）。

## 交付物
- **存量项目**：版本目录 docs/{项目英文缩写}-v0.0.1/ 及其全部初始化文档、主文档（project.md、sad.md、urs.md、prd.md、api.md、dbd.md、dbd.sql、lld.md、testcase.md）、任务清单、版本进度表 version_progress.md、.gitignore 与 git 初始提交记录、接口测试用例与执行器、各文档生成后的用户审核确认记录（对话内完成，摘要不落盘）
- **空项目**：docs/project.md（已审核）、.gitignore 与 git 初始提交记录

## 完成后提示
- **存量项目**：初始化阶段已全部完成（含逐文档审核），下一步进入 impm-docs-review 需求分析整理阶段（输入 /impm-docs-review）。如需查看版本进度，请查看版本目录下的 version_progress.md。
- **空项目**：空项目初始化已完成（git + project.md，project.md 已审核通过），将直接进入 impm-docs-review 需求分析整理阶段。
