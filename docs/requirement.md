...
- 处理内容：
  ...
  4、不要将完整内容复制到docs/{项目英文缩写}-urs.md，而是从当前版本URS中提取摘要（需求编号/名称/描述摘要/优先级、业务目标/场景/约束/假设概要，含版本演进表与完整文档索引）写入docs/{项目英文缩写}-urs.md。摘要正文**不按版本号分节**，按「项目 → 模块 → 业务场景」重组并按业务逻辑排序（主流程在前、支线在后），版本信息由来源版本列与版本演进表承载。完整内容保留在版本目录文档。
...
- 处理内容：
  ...
  4、不要将完整内容复制到docs/{项目英文缩写}-prd.md，而是从当前版本PRD中提取摘要（功能清单F编号/名称/模块/优先级/版本范围、用户故事US编号/标题/摘要/优先级、产品背景/目标用户/数据需求/验收标准概要，含版本演进表与完整文档索引）写入docs/{项目英文缩写}-prd.md。摘要正文**不按版本号分节**，功能清单按「项目 → 模块」分组、模块内按业务逻辑排序，用户故事按所属功能/模块分组并逻辑排序，版本信息由来源版本/版本范围列与版本演进表承载。完整内容保留在版本目录文档。
...
### i) 生成需求追踪矩阵RTM文档
- Skill：impm-rtm-create
- agent：TL
- 处理内容：
  1、从模板目录下读取RTM-TEMPLATE.MD模板文件。
  2、根据当前版本的URS（需求编号FR/NFR）、PRD（用户故事编号US）、LLD（设计内容）和task任务清单，建立"需求 → 设计 → 任务"的多对多追踪矩阵（一条关联即一行，同一需求/用户故事可出现多行）：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-rtm-v{当前版本号}.md。
  3、执行覆盖完整度校验：每个需求/用户故事是否有设计、是否有任务（测试用例留待回归测试阶段回填），缺失项填入RTM问题清单。
  4、版本进度文件：docs/{项目英文缩写}-v{当前版本号}/version_progress.md。增加第一行：步骤序号：前一序号+1，步骤名称：impm-rtm-create，步骤状态：已完成。

### j) 需求分析阶段内容提交
- Skill：impm-analysis-commit
- agent：SCM
- 处理内容：将需求分析整理阶段生成的这些文件和目录（含 rtm.md）都提交到git上。
...
- 处理内容：合并采用「重构式合并」，按项目组织方式、代码结构和系统架构组织主文档结构，而非简单追加。
  1. URS：docs/{项目英文缩写}-v{当前版本号}/ {项目英文缩写}-urs-v{当前版本号}.md 合并到docs/{项目英文缩写}-urs.md，只合并**摘要**（需求编号FR/NFR、名称、一行描述、优先级、业务归属），**不按版本号分节**，按「项目 → 模块 → 业务场景」重组并按业务逻辑排序（主流程在前、支线在后），版本信息由来源版本列与版本演进表承载；不合并完整内容；主文档不存在就先创建。完整内容保留在版本目录文档。
